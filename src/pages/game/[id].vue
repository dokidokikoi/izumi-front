  ...
  gameApi.getIns(gameId.value).then((res) => {
    ...
    for (const ins of res.data) {
      for (const plat of ins.platform) {
        if (!platforms.value.indexOf(plat) === -1) {
          platforms.value.push(plat)
        }
      }
      for (const lang of ins.language) {
        if (!languages.value.indexOf(lang) === -1) {
      ...
      <div class="flex flex-col gap-6 md:flex-row">
        ...
        <div class="flex-1 space-y-3">
          <div flex items-start>
            ...
            <div flex-1>
              <template v-if="!gameStore.showEdit">
                <div v-for="alias in game?.alias" :key="alias">
          ...
          <div flex items-center>
            ...
            <div flex-1>
              <el-select v-model="gameInsIdx" placeholder="版本" :empty-values="[null, undefined, 0]" class="mr-2 w64" clearable>
                <el-option
          ...
          <div flex items-center>
            ...
            <div flex-1>
              <span
                v-if="!gameStore.showEdit" cursor-pointer text-lg underline-blue-6 hover:text-blue-500 hover:underline
                @click="router.push(`/game?category=${game?.category?.id}`)"
          ...
          <div flex items-start>
            ...
            <div flex-1>
              <template v-if="!gameStore.showEdit">
                <p
                  v-for="s in game.series" :key="s.id" cursor-pointer text-lg underline-blue-6 hover:text-blue-500 hover:underline
                  @click="router.push(`/game?series=${s.id}`)"
                >
                  {{ s.name }}
                </p>
              </template>
          ...
          <div v-if="gameInsIdx">
            ...
            <div flex-1>
              <span text-lg>{{ gameIns[gameInsIdx - 1].path }}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="flex flex-col gap-6 md:flex-row">
        ...
        <div class="flex-1 space-y-3">
          ...
          <div flex items-center>
            ...
            <div flex-1>
              <span v-if="!gameStore.showEdit" text-lg hover:text-blue-500 cursor-pointer underline-blue-6 hover:underline
                @click="router.push(`/game?brand=${game?.brand?.id}`)"
              >{{ game?.brand?.name }}</span>
              <div v-else flex flex-1 flex-wrap items-center>
                <el-select v-model="createBrandID" placeholder="游戏品牌" :empty-values="[null, undefined, 0]" mr-2 w-64>
          ...
          <div flex items-start>
            ...
            <div flex-1>
              <template v-if="!gameStore.showEdit">
                <p v-for="s in game.series" :key="s.id" text-lg hover:text-blue-500 cursor-pointer underline-blue-6 hover:underline
                  @click="router.push(`/game?series=${s.id}`)"
                >
                  {{ s.name }}
                </p>
          ...
          <div flex items-start>
            ...
            <div flex-1>
              <template v-if="!gameStore.showEdit">
                <span
                  v-for="tag in game.tags" :key="tag.id"
                  class="mb-1 mr-1 inline-block cursor-pointer whitespace-nowrap border border-gray-600 rounded px-2 py-1 text-sm dark:border-white hover:text-blue-500 hover:border-blue-500"
                  @click="router.push(`/game?tags=${tag.name}`)"
                >
                  {{ tag.name }}
                </span>
      ...
      <div class="flex flex-col gap-6 md:flex-row">
        ...
        <div class="flex-1 space-y-3">
          ...
          <div flex items-center>
            ...
            <div flex-1>
              <span v-if="!gameStore.showEdit"      cursor-pointer text-lg underline-blue-6 hover:text-blue-500 hover:underline
                @click="router.push(`/game?brand=${game?.brand?.id}`)"
              >{{ game?.brand?.name }}</span>
              <div v-else flex flex-1 flex-wrap items-center>
                <el-select v-model="createBrandID" placeholder="游戏品牌" :empty-values="[null, undefined, 0]" mr-2 w-64>
          ...
          <div flex items-start>
            ...
            <div flex-1>
              <template v-if="!gameStore.showEdit">
                <p v-for="s in game.series" :key="s.id"      cursor-pointer text-lg underline-blue-6 hover:text-blue-500 hover:underline
                  @click="router.push(`/game?series=${s.id}`)"
                >
                  {{ s.name }}
                </p>
